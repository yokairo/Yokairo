...
$$('[data-page]').forEach(button => button.addEventListener('click', () => { const page = button.dataset.page; $$('.page-panel').forEach(panel => panel.classList.toggle('active', panel.id === page)); $$('nav button').forEach(item => item.classList.toggle('active', item === button)); document.body.dataset.page = page; window.scrollTo({ top: 0, behavior: 'smooth' }); }));
$('#enterOrbit').onclick = () => document.querySelector('nav button[data-page="orbit"]').click();
